...
if not MAIN_BOT_TOKEN:
    logging.error("❌ MAIN_BOT_TOKEN is missing! Please add it to Railway Variables.")
    sys.exit(1)
...
async def check_sub_callback(callback: types.CallbackQuery):
    if await check_subscription(callback.from_user.id):
        await callback.answer("✅ شكراً لاشتراكك!")
...
async def broadcast_handler(message: types.Message):
    ...
    if not message.reply_to_message and not broadcast_text:
        await message.answer("❌ يرجى كتابة نص الإذاعة بعد الأمر أو الرد بالأمر على رسالة.")
        return
    ...
    if not users:
        await message.answer("❌ لا يوجد مستخدمون للإرسال لهم.")
        return

    msg = await message.answer(f"🚀 بدأت الإذاعة لـ {len(users)} مستخدم...")
    
    success, failed = 0, 0
    for user in users:
        try:
            if message.reply_to_message:
                await target_message.copy_to(chat_id=user[0])
            else:
    ...
    await msg.edit_text(f"📊 **تقرير الإذاعة:**\n\n✅ نجاح: {success}\n❌ فشل: {failed}")
